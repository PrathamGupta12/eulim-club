...
function SponsorStrip({ sponsors, title }: { sponsors: typeof exhibitionSponsors; title: string }) {
  return (
    <motion.div
      ...
      className="mt-12"
    >
      <h4 className="activities-theme-kicker text-sm font-semibold text-muted-foreground tracking-widest uppercase text-center mb-6">{title}</h4>
      <div className="flex flex-wrap justify-center items-center gap-4 md:gap-10">
        {sponsors.map((s) => (
          ...
          >
            <div className="activities-theme-card h-16 w-16 md:h-20 md:w-20 rounded-xl bg-card border border-border overflow-hidden flex items-center justify-center group-hover:border-primary/40 transition-colors p-2">
